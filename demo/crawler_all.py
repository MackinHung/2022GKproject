...
def crawler_all(keyword,page_num,year_start,year_end):
#媽咪愛爬蟲
...
# keyword = '珍珠卵磷脂'
        ...
        for month in range(1,13,3):
            for date in dates:
                try:
                    ...
                    #不斷更改網址 不斷前往網站爬取
                    url = 'https://tw.observer/api/posts?hot=0&before='+str(year)+'-'+str(month)+'-'+date+'&term='+keyword
                    ...
                    for i in range(len(rx)):
                        posts = []
                        dc_order = rx[i]['order']
                        dc_order = ILLEGAL_CHARACTERS_RE.sub(r'', dc_order)
                        posts.append(dc_order)
                        dc_title = rx[i]['title']
                        dc_title = ILLEGAL_CHARACTERS_RE.sub(r'', dc_title)
                        posts.append(dc_title)
                        dc_content = rx[i]['content']
                        dc_content = ILLEGAL_CHARACTERS_RE.sub(r'', dc_content)
                        posts.append(dc_content)
                        posts.append(rx[i]['commentCount'])
                        posts.append(rx[i]['likeCount'])
                        dc_forumName = rx[i]['forumName']
                        dc_forumName = ILLEGAL_CHARACTERS_RE.sub(r'', dc_forumName)
                        posts.append(dc_forumName)
        ...
        for n in page_urls :
            ...
            try:
                for comment in comments:
                    ...
                    push_tag = ILLEGAL_CHARACTERS_RE.sub(r'', push_tag)
                    push_userid = ILLEGAL_CHARACTERS_RE.sub(r'', push_userid)
                    push_content = ILLEGAL_CHARACTERS_RE.sub(r'', push_content)
                    push_time = ILLEGAL_CHARACTERS_RE.sub(r'', push_time)
            except TypeError:
                print(TypeError)
    ...
    filename1 = raw_folder + '/' + keyword+'_媽咪愛_網路爬蟲.xlsx'
    ...
    filename2 = raw_folder + '/' + keyword+'_媽咪拜_網路爬蟲.xlsx'
    title =["文章標題","文章內容"]
