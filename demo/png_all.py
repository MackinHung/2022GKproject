def png_all(keyword_png,brand1,brand2,brand3,brand4,brand5,brand6,brand7,brand8,brand9,brand10):#, brandlist = None):
    import requests # 訪問
    import jieba
    ...
    import os
    import csv
    # def h(keyword,*brands, brandlist = None):
    # if brandlist is None: brandlist = []

    brandlist = []
    if brand1 != '':
        brandlist.append(brand1)
    if brand2 != '':
        brandlist.append(brand2)
    if brand3 != '':
        brandlist.append(brand3)
    if brand4 != '':
        brandlist.append(brand4)
    if brand5 != '':
        brandlist.append(brand5)
    if brand6 != '':
        brandlist.append(brand6)
    if brand7 != '':
        brandlist.append(brand7)
    if brand8 != '':
        brandlist.append(brand8)
    if brand9 != '':
        brandlist.append(brand9)
    if brand10 != '':
        brandlist.append(brand10)
    ...
    # for keywords in brandlist:
    # #讀取檔案
    #     print('keywords',keywords)
    # -*- coding: utf-8 -*-
    web = 'total'
    raw_folder = keyword_png+'/'
    all_article = raw_folder +keyword_png + '_'  + web +'_網路爬蟲.txt'
    all_article_edited = raw_folder +keyword_png + '_' + web +'_edited_text.txt'
    dict_path='結巴套件資料/'
    ...
    # 分析圖表創建資料夾後
    # 將檔案存入
    myWordClode.to_file(images_folder +keyword_png + '_文字雲.png')
    print('文字雲完成繪畫及儲存')
    ...
    for brand in brandlist:
        ...
        # print(brand)
        try:
            with open(brand +'/' + brand + '_'  + web +'_網路爬蟲.txt', encoding="utf-8", errors='ignore') as f:
                text_s = f.read()
        except FileNotFoundError as fnf:
            print(fnf)
        except IOError as iof:
    ...
    plt.figure(figsize=(9,7))
    plt.bar(brandlist, h,color="#A9D08E",label="正向評價")
    plt.bar(brandlist, b,color="#9BC2EF",bottom=h,label="負向評價")
    ...
    plt.savefig(images_folder + keyword_png + '_正負評價比_各品牌.png') #要在show之前
    ...
    try:
        ...
        dictionary = dict(zip(brandlist,keyword_values))
        # print ("按值(value)排序:")  
        ...
        ax.set_title(keyword_png +'品牌網路聲量圖',fontweight='bold',fontsize = 16)
    ...
    plt.savefig(images_folder + keyword_png +'_品牌網路聲量圖.png')
    # plt.show()
...
# # 新增及刪除常用詞
    ...
    try:
        ...
        #,explode=explode)
        # ax0.legend(loc = "best")
        ax0.set_title('網友在意的' + keyword_png + '面向',fontweight='bold',fontsize = 16)
        ...
        print (v)
    plt.savefig( images_folder + keyword_png + '_網友在意面向圖.png') #要在show之前
#     # plt.show()
...
# keyword_png ='滴雞精'
# png_all(keyword,'農純鄉','大地之愛','風車生活','和春堂')
# png_all(keyword,'不在家','','','','','','','','','')
# png_all(keyword_png,'農純鄉','純煉','老協珍','田原香','白蘭氏','娘家','桂格','','','')